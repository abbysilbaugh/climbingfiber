...
function plotPairedPrePost(RWS_pre, RWS_post, RWSCF_pre, RWSCF_post, figtitle, choosetype)
    ...
    figure('Position', [680,645,120,333]);
...
function p = plotOnePair(preData, postData, choosetype)
    ...
    getidx = isinf(preData) | isnan(preData) | isinf(postData) | isnan(postData);
    preData = preData(~getidx);
    postData = postData(~getidx);

    m_pre  = mean(preData);
    m_post = mean(postData);
    sem_pre  = std(preData)  / sqrt(sum(~isnan(preData)));
    sem_post = std(postData) / sqrt(sum(~isnan(postData)));
