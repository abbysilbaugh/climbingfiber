function timegraph_auc_V2(binned_RWS, binned_RWSCF, evoked_win, cellType, neuron_colors, ylimits, ploterrorbars, xlimit, endANOVA, choosepersistent)
...
% Pool
pooledRWSCF = NaN(nRWSCFcells, 24);
for j = 1:size(binned_RWSCF, 2)
    pool = [];
    for i = 1:size(binned_RWSCF, 1)
        temp = binned_RWSCF{i, j};
...
if strcmp(choosepersistent, 'persistent')
    pre_RWS = pooledRWS(:, 1:13);
    post_RWS = pooledRWS(:, 14:end);
    ...
    pre_RWSCF = pooledRWSCF(:, 1:13);
    post_RWSCF = pooledRWSCF(:, 14:end);
...
temp = zeros(size(binned_RWS, 1), 24);
for i = 1:size(binned_RWS, 1)
for j = 1:24
...
temp = zeros(size(binned_RWSCF, 1), 24);
for i = 1:size(binned_RWSCF, 1)
for j = 1:24
...
RWS_numcells = zeros(24, 1);
RWSCF_numcells = zeros(24, 1);
for i = 1:24
...
pooledRWS = pooledRWS(:, 12:endANOVA);
pooledRWSCF = pooledRWSCF(:, 12:endANOVA);
