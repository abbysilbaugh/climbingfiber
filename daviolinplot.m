...
for g = 1:num_groups 
    ...
    for k = 1:num_locs
        ...
        % Define xi range within the min and max of data
        ...
        set(h.ds(k,g),'FaceAlpha',confs.violinalpha, 'EdgeAlpha', '1', 'LineWidth', .75, 'EdgeColor', confs.colors(g,:)); hold on
        ...
        elseif confs.jitter==1
            xdata =  gpos(g,k).*ones(numel(Y(Gi==g,k)),1) + ...
                (box_width/.5).*(0.5 - rand(numel(Y(Gi==g,k)),1));
                ...
                if confs.withinlines==1
                    for s = 1:size(scdata{g},1)                
                        h.wl(g) = plot(squeeze(scdata{g}(s,1,:)),...
                            squeeze(scdata{g}(s,2,:)),'color', [0.8 0.8 0.8, 0.5], 'LineWidth', 0.3);
