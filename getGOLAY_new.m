...
function [allData] = getGOLAY_new(allData, b_percent, sliding_window_size, smooth_win, whichbackgroundmethod)
...
%     end
    ...
    % Method 3: for each cell, find b_percent prctile value of average trace (1 value)
        % Output: golaysignal3
    smoothcorrsignal3 = zeros(nFrames, nCells, nTrials);
    for k = 1:nCells
        ...
        for j = 1:nTrials
            if F0 < 0
                smoothcorrsignal3(:, k, j) = NaN(nFrames, 1);
            else
                smoothsignal_temp = smoothsignal(:, k, j);
                smoothsignal_temp = (smoothsignal_temp - F0)./F0;
                smoothcorrsignal3(:, k, j) = smoothsignal_temp;
            end
        end
        if F0 < 0
...
%     end
    ...
    allData.data{i}.golaysignal3 = smoothcorrsignal3;
