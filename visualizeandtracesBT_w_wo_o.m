...
function [pre, pre_wo, pre_o] = visualizeandtracesBT_w_wo_o(neuron_colors, xlimits, ylimits, stimframe,...
    pre, pre_wo, pre_o, showtraces)
...
for i = 1:length(neuron_names)
        ...
        pre_w_selected = pre{i};
        pre_wo_selected = pre_wo{i};
        pre_o_selected = pre_o{i};
        
        findcells = ~isnan(pre_w_selected(1, :)) & ~isnan(pre_wo_selected(1,:)) & ~isnan(pre_o_selected(1,:));
        
        pre_w_selected = pre_w_selected(:, findcells);
        pre_wo_selected = pre_wo_selected(:, findcells);
        pre_o_selected = pre_o_selected(:, findcells);

        pre{i} = pre_w_selected;
        pre_wo{i} = pre_wo_selected;
        pre_o{i} = pre_o_selected;
        
        getmean_pre = mean(pre_w_selected, 2, 'omitnan');
        getmean_pre_wo = mean(pre_wo_selected, 2, 'omitnan');
        getmean_pre_o = mean(pre_o_selected, 2, 'omitnan');
        
        getsem_pre = mean(pre_w_selected, 2, 'omitnan') / sqrt(size(pre_w_selected, 2));
        getsem_pre_wo = mean(pre_wo_selected, 2, 'omitnan') / sqrt(size(pre_wo_selected, 2));
        getsem_pre_o = mean(pre_o_selected, 2, 'omitnan') / sqrt(size(pre_o_selected, 2));
        ...
        if strcmp(showtraces, 'yesplot')
            ...
            plot(getmean_pre_o, 'Color', 'k', 'LineWidth', 0.5)
            fill([1:length(getmean_pre_o), length(getmean_pre_o):-1:1], [getmean_pre_o - getsem_pre_o; flipud(getmean_pre_o + getsem_pre_o)], [1 1 1], 'linestyle', 'none', 'FaceAlpha', 0.5);
            xlim(xlimits)
            ylim(ylimits)
