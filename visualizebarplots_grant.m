...
function [increase_cells, decrease_cells, active_PRE_RWS, active_POST_RWS] = visualizebarplots_grant(allData, selectmottype, evoked_win, max_win, plottype, ylimit)
...
%     ylim([ylimit])
    ...
    % Compare same cells
    for k = 1:length(pks_pre_RWS)
        pre = pks_pre_RWS(k);
        post = pks_post_RWS(k);
        if isnan(pre) & ~isnan(post)
            pks_pre_RWS(k) = 0;
        elseif ~isnan(pre) & isnan(post)
            pks_post_RWS(k) = 0;
        end
    end
    ...
    figure;
    hold on;
    bar([1,2], [mean(pks_pre_RWS, 'omitnan'), mean(pks_post_RWS, 'omitnan')], 0.5, 'FaceColor', colors{counter}(1:3), 'FaceAlpha', 0.5, 'EdgeColor', 'none', 'BarWidth', 0.5);
...
%     end
    ...
    getactive_pre = ~isnan(pks_pre_RWS); getactive_pre = pks_pre_RWS(getactive_pre);
    getactive_post = ~isnan(pks_post_RWS); getactive_post = pks_post_RWS(getactive_post);
    [~, p, ~, ~] = ttest(getactive_pre, getactive_post)

    % Plot line above error bars with asterisk if p-value is significant
    if p < 0.05
        y = max([mean(pks_pre_RWS, 'omitnan') + std(pks_pre_RWS, 'omitnan')/sqrt(n_pre_RWS), mean(pks_post_RWS, 'omitnan') + std(pks_post_RWS, 'omitnan')/sqrt(n_post_RWS)]);
