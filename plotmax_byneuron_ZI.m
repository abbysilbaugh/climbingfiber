% Specify whether traces have been trial-averaged
    % If yes, calculates max based on trial-averaged traces
    % If no, calculates max from each trial, then trial-averages max

function plotmax_byneuron_ZI(m1_cond1, m1_cond2, m2_cond1, m2_cond2, neuron_colors, plotlines, evoked_win, istrialaveraged)
% Enter as w_pre, w__dcz, wo_pre, wo_dcz
% m1 is whisker, m2 is whiskerOpto
% cond1 is pre, cond2 is after dcz administration
if strcmp(istrialaveraged, 'istrialaveraged')
    % Get event probability (n responses/ntrials) for each neuron
    m1_cond1 = getmax_ta(m1_cond1, evoked_win);
    m1_cond2 = getmax_ta(m1_cond2, evoked_win);
    m2_cond1 = getmax_ta(m2_cond1, evoked_win);
    m2_cond2 = getmax_ta(m2_cond2, evoked_win);

    % Reformat data
    % Whisker stimulus; column 1 is cond1 (pre) and column 2 is cond2 (dcz)
    w_PV = reformatdata_ta(m1_cond1, m1_cond2, 1);
    w_PN = reformatdata_ta(m1_cond1, m1_cond2, 2);
    w_VIP = reformatdata_ta(m1_cond1, m1_cond2, 3);
    w_UC = reformatdata_ta(m1_cond1, m1_cond2, 4);
    w_SST = reformatdata_ta(m1_cond1, m1_cond2, 5);
    
    % whiskerOpto stimulus; column 1 is cond1 and column 2 is cond2 (dcz)
    wo_PV = reformatdata_ta(m2_cond1, m2_cond2, 1);
    wo_PN = reformatdata_ta(m2_cond1, m2_cond2, 2);
    wo_VIP = reformatdata_ta(m2_cond1, m2_cond2, 3);
    wo_UC = reformatdata_ta(m2_cond1, m2_cond2, 4);
    wo_SST = reformatdata_ta(m2_cond1, m2_cond2, 5);
    
    % pre; column 1 is whisker and column 2 is whiskerOpto
    pre_PV = reformatdata_ta(m1_cond1, m2_cond1, 1);
    pre_PN = reformatdata_ta(m1_cond1, m2_cond1, 2);
    pre_VIP = reformatdata_ta(m1_cond1, m2_cond1, 3);
    pre_UC = reformatdata_ta(m1_cond1, m2_cond1, 4);
    pre_SST = reformatdata_ta(m1_cond1, m2_cond1, 5);
    
    % dcz; column 1 is whisker and column 2 is whiskerOpto
    dcz_PV = reformatdata_ta(m1_cond2, m2_cond2, 1);
    dcz_PN = reformatdata_ta(m1_cond2, m2_cond2, 2);
    dcz_VIP = reformatdata_ta(m1_cond2, m2_cond2, 3);
    dcz_UC = reformatdata_ta(m1_cond2, m2_cond2, 4);
    dcz_SST = reformatdata_ta(m1_cond2, m2_cond2, 5);
else
    % Get event probability (n responses/ntrials) for each neuron
...
function output = reformatdata_ta(var1, var2, celltype)
        counter = 1;
        output = [];
        for i = 1:size(var1, 1)
            temp = var1{i, celltype};
...
function var = getmax_ta(var, evoked_win)
[nMice, ncellType] = size(var);
    for i = 1:nMice
        for j = 1:ncellType
            signal = var{i, j};
