...
function visualizeevokedtracesORcells(allData, selectmottype, evoked_win)
window = (evoked_win(2) - evoked_win(1))*32.25;
colors = {[187/255, 37/255, 37/255]};
...
nFrames = size(allData.data{1}.ac_bs_signal, 1);
 ...
 for counter = 1:length(neurontypes)
     ...
     wpre_wo_mice_all = cell(nMice, 1);
     wopre_wo_mice_all = cell(nMice, 1);
     ...
     for i = wo_mice
         ...
         getvar = allData.data{i}.golayshift(:, getcells, w_pre_trials{i}); getvar = mean(getvar, 3, 'omitnan');
         wpre_wo_mice_all{i} = getvar;
         getvar = allData.data{i}.golayshift(:, getcells, wo_pre_trials{i});  getvar = mean(getvar, 3, 'omitnan');
         wopre_wo_mice_all{i} = getvar;
     ...
     wpre_RWS_mice_all = cell(nMice, 1);
     wpost_RWS_mice_all = cell(nMice, 1);
     ...
     for i = RWS_mice
         ...
         getvar = allData.data{i}.golayshift(:, getcells, w_pre_trials{i});  getvar = mean(getvar, 3, 'omitnan');
         wpre_RWS_mice_all{i} = getvar;
         getvar = allData.data{i}.golayshift(:, getcells, w_post_trials{i});  getvar = mean(getvar, 3, 'omitnan');
         wpost_RWS_mice_all{i} = getvar;
     ...
     wpre_RWSCF_mice_all = cell(nMice, 1);
     wpost_RWSCF_mice_all = cell(nMice, 1);
     ...
     for i = RWSCF_mice
         ...
         getvar = allData.data{i}.golayshift(:, getcells, w_pre_trials{i});  getvar = mean(getvar, 3, 'omitnan');
         wpre_RWSCF_mice_all{i} = getvar;
         getvar = allData.data{i}.golayshift(:, getcells, w_post_trials{i});  getvar = mean(getvar, 3, 'omitnan');
         wpost_RWSCF_mice_all{i} = getvar;
     ...
     w_all = zeros(nFrames, nCells_wo_mice);
     wo_all = zeros(nFrames, nCells_wo_mice);
     ...
     for n = 1:nMice
         ...
         temp = wpre_wo_mice_all{n};
         ncells = size(temp, 2);
         w_all(:, c:c+ncells-1) = temp;
         temp = wopre_wo_mice_all{n};
         wo_all(:, c:c+ncells-1) = temp;
         c = c + ncells;
         end
     end
     ...
     pre_RWS_all = zeros(nFrames, nCells_RWS_mice);
     post_RWS_all = zeros(nFrames, nCells_RWS_mice);
     c = 1;
     for n = 1:nMice
         if ~isempty(wpre_RWS_mice{n})
         ...
         temp = wpre_RWS_mice_all{n};
         ncells = size(temp, 2);
         pre_RWS_all(:, c:c+ncells-1) = temp;
         temp = wpost_RWS_mice_all{n};
         post_RWS_all(:, c:c+ncells-1) = temp;
         c = c + ncells;
         end
     end
     ...
     for n = 1:nMice
         ...
         temp = wpre_RWSCF_mice_all{n};
         ncells = size(temp, 2);
         pre_RWSCF_all(:, c:c+ncells-1) = temp;
         temp = wpost_RWSCF_mice_all{n};
         post_RWSCF_all(:, c:c+ncells-1) = temp;
         c = c + ncells;
         end
     end
    
    subplot(5, 3, imagecount)
    getactivecells = ~isnan(w(1, :));
    getactivecells2 = ~isnan(wo(1, :));
    getactivecells = getactivecells + getactivecells2; getactivecells = getactivecells > 0; num = sum(getactivecells);
    w_mean = mean(w_all(:, getactivecells), 2);
    w_sem = std(w_all(:, getactivecells), 0, 2) / sqrt(num);
    wo_mean = mean(wo_all(:, getactivecells), 2);
    wo_sem = std(wo_all(:, getactivecells), 0, 2) / sqrt(num);
    hold on
    ...
    legend([h1, h2], ['W: ', num2str(num)], ['W+CF: ', num2str(num)], 'box', 'off', 'Location', 'northwest')
    title(['Basic Transmission (', num2str(window), ' ms); nCells = ', num2str(length(getactivecells))])
    ylim([0 1])
    ylabel('ΔF/F0')
    ...
    subplot(5, 3, imagecount)
    getactivecells = ~isnan(pre_RWS(1, :)); 
    getactivecells2 = ~isnan(post_RWS(1, :)); 
    getactivecells = getactivecells + getactivecells2; getactivecells = getactivecells > 0; num = sum(getactivecells);

    pre_RWS_mean = mean(pre_RWS_all(:, getactivecells), 2);
    pre_RWS_sem = std(pre_RWS_all(:, getactivecells), 0, 2) / sqrt(num); % SEM calculation
    post_RWS_mean = mean(post_RWS_all(:, getactivecells), 2);
    post_RWS_sem = std(post_RWS_all(:, getactivecells), 0, 2) / sqrt(num); % SEM calculation
    ...
    imagecount = imagecount + 1;
    title(['RWS (', num2str(window), ' ms) ; nCells = ', num2str(length(getactivecells))])
    legend([h1, h2], ['Pre: ', num2str(num)], ['Post: ', num2str(num)], 'box', 'off', 'Location', 'northwest')
    ylim([0 1])
    xticks([0 62 124 186 248 310 372 434 496 558 620])
    xticklabels([-10 -8 -6 -4 -2 0 2 4 6 8 10])
    xlabel('Time Relative to Stimulus Onset (s)')
    ...
    subplot(5, 3, imagecount)
    getactivecells = ~isnan(pre_RWSCF(1, :));
    getactivecells2 = ~isnan(post_RWSCF(1, :));
    getactivecells = getactivecells + getactivecells2; getactivecells = getactivecells > 0; num = sum(getactivecells);

    pre_RWSCF_mean = mean(pre_RWSCF_all(:, getactivecells), 2);
    pre_RWSCF_sem = std(pre_RWSCF_all(:, getactivecells), 0, 2) / sqrt(num); % SEM calculation
    post_RWSCF_mean = mean(post_RWSCF_all(:, getactivecells), 2);
    post_RWSCF_sem = std(post_RWSCF_all(:, getactivecells), 0, 2) / sqrt(num); % SEM calculation
    ...
    imagecount = imagecount + 1;
    legend([h1, h2], ['Pre: ', num2str(num)], ['Post: ', num2str(num)], 'box', 'off', 'Location', 'northwest')
    title(['RWSCF (', num2str(window), ' ms) ; nCells = ', num2str(length(getactivecells))])
    ylim([0 1])
    xticks([0 62 124 186 248 310 372 434 496 558 620])
    xticklabels([-10 -8 -6 -4 -2 0 2 4 6 8 10])
    xlabel('Time Relative to Stimulus Onset (s)')
    ylabel('ΔF/F0')
     ...
     figure('Position', [50 50 1800 800/3])
