...
nFrames = size(allData.data{1}.ac_bs_signal, 1);
 ...
 for counter = 1:length(neurontypes)
    ...
    getactivecells = getactivecells & getactivecells2; num = sum(getactivecells);
    w_mean = mean(w(:, getactivecells), 2);
    w_sem = std(w(:, getactivecells), 0, 2) / sqrt(num);
    wo_mean = mean(wo(:, getactivecells), 2);
    wo_sem = std(wo(:, getactivecells), 0, 2) / sqrt(num);
    hold on
    ...
    pre_RWS_sem = std(pre_RWS(:, getactivecells), 0, 2) / sqrt(num); % SEM calculation
    post_RWS_mean = mean(post_RWS(:, getactivecells), 2);
    post_RWS_sem = std(post_RWS(:, getactivecells), 0, 2) / sqrt(num); % SEM calculation
    ...
    pre_RWSCF_sem = std(pre_RWSCF(:, getactivecells), 0, 2) / sqrt(num); % SEM calculation
    post_RWSCF_mean = mean(post_RWSCF(:, getactivecells), 2);
    post_RWSCF_sem = std(post_RWSCF(:, getactivecells), 0, 2) / sqrt(num); % SEM calculation
