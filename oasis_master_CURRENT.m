...
% Only 'figs' folder can exist in 20230516_analysis

%% ADD DEFAULT ARTIFACT FRAMES FOR NEW MICE
% artifactframes = [312];
% save('20240807_artifactframes.mat', 'artifactframes')
...
allData = getGOLAY_new(allData, 1, 5, 15, 'rawminusfirstprctile');

%%
visualizesmoothing(allData)
% visualizesmoothing_individualneurons(allData, 26, 'golaysignal3', [-0.5, 2], [279 372],...
%     'ploteachtrial', 'all', baselineshiftperiod, baselineshift, baseline_reject_thresh);
%% PROCESS MOTION DATA (REST, RUNNING; M, NM, T, H TRIALS)
...
[allData] = gettransition(allData, [304 310], [310 316], NM_thresh, T_thresh, M_thresh, H_thresh);
%%
%visualizemotion(allData, 'all')
%%
%visualizemotion_byperiod(allData, [279, 372])

%% CALCULATE BASELINE NOISE (σ) and SNR

[baselineNoise, SNR] = getbaselinenoise_fast(allData, 15, 95, 'golaysignal', 10);
[baselineNoise2, SNR2] = getbaselinenoise_fast(allData, 15, 95, 'golaysignal2', 10);
[baselineNoise3, SNR3] = getbaselinenoise_fast(allData, 15, 95, 'golaysignal3', 10);
...
[allData] = getpeaks_new(allData, 2, baselineNoise, 305);
[allData] = getallpeaks(allData, 2, baselineNoise);
%visualizeallpeaksbymouse(allData)
%visualizepooledspikes(allData, 'PN', [310, 340], 'NM')
%visualizepooledpeaks(allData, 'PN', [310, 340], 'NM')
%visualizetrialavgpeaks(allData, baselineNoise, 'PN', [310, 314])
...
evoked_win = [305 320];
[allData] = getevokedtraces(allData, evoked_win);
visualizeevokedtraces(allData, 'NM', evoked_win)
%visualizeevokedtracesORcells(allData, 'NM', evoked_win)
%visualizeevokedtracesORcells_DREADD(allData, 'NM', evoked_win)
%visualizeevokedtracesANDcells(allData, 'all', evoked_win)
...
%% PARSE DATA
RWS_mice = [2, 3, 4, 5, 10, 11, 18, 20];
RWSCF_mice = [33, 34, 35]; % add in JAWS!!!
%RWSCF_mice = [1, 6, 7, 8, 9, 13, 14, 15, 27, 28, 31];
RWSCF_PV_Gi_mice = [22, 23];
control_PV_Gi_mice = [24, 25];
RWSCF_SST_Gi_mice = [29]; %19
control_SST_Gi_mice = [30, 32]; % [30, 32];
RWS_SST_Gq_mice = [19];
P_JAWS_control = 36;
...
[pre, pre_wo, ...
    ...
    = parseexperiments(baselineshiftperiod, baselineshift, baseline_reject_thresh, trialavg,...
    cellavg, mouseavg, 'all', allData, 'golaysignal3', RWS_mice, RWSCF_mice, RWSCF_PV_Gi_mice, control_PV_Gi_mice, RWSCF_SST_Gi_mice, control_SST_Gi_mice);
%%
visualizeparseddata([289.5, 382.5], [-0.1, 0.3], 303, ...
    pre, pre_wo, ...
    RWS_pre, RWS_pre_wo, RWS_post, ...
    RWSCF_pre, RWSCF_pre_wo, RWSCF_post, ...
    RWSCF_PV_Gi_pre, RWSCF_PV_Gi_pre_wo, RWSCF_PV_Gi_pre_dcz, RWSCF_PV_Gi_post, ...
...
for i = 1:length(allData.data)
    plottimeseries(allData, i, 'PN', 'all')
end
%%

for i = 1:length(allData.data)
    plottimeseriesDREADD(allData, i, 'PN', 'all')
end
...
peak_win = {false, 10};
spike_win = {false, 3}; % CHANGED FROM 10 Nov 5, CHANGED FROM 4 2/11/24 (4)
...
gettitle = {[], []};
exclude = false; % switched 01/28/24
