...
function visualizeviolinplots(allData, selectmottype, evoked_win, max_win, plottype, ylimit)
...
nFrames = size(allData.data{1}.ac_bs_signal, 1);
 ...
 for counter = 1:length(neurontypes)
    ...
    hold on;
    if n_w > 0 && n_wo > 0
        violinplot([pks_w, pks_wo], {'W', 'W+CF'}, 'ViolinColor', colors{counter}(1:3));
        ylabel('∆F/F0');
        [stats] = runstats(pks_w, pks_wo, 'unpaired');
        title(['p = ', num2str(stats.p), '; effectSize = ', num2str(stats.effectSize), '; ' stats.normality]);
        hold off;
    end
%    ylim(ylimit);
    ...
    subplot(1,3,2);
    hold on;
    if n_pre_RWS > 0 && n_post_RWS > 0
        violinplot([pks_pre_RWS, pks_post_RWS], {'Pre', 'Post'}, 'ViolinColor', colors{counter}(1:3));
        ylabel('∆F/F0');
        [stats] = runstats(pks_pre_RWS, pks_post_RWS, 'unpaired');
        title(['p = ', num2str(stats.p), '; effectSize = ', num2str(stats.effectSize), '; ' stats.normality]);
        hold off;
    end
   % ylim(ylimit);
    ...
    subplot(1,3,3);
    hold on;
    if n_pre_RWSCF > 0 && n_post_RWSCF > 0
        violinplot([pks_pre_RWSCF, pks_post_RWSCF], {'Pre', 'Post'}, 'ViolinColor', colors{counter}(1:3));
        ylabel('∆F/F0');
        [stats] = runstats(pks_pre_RWSCF, pks_post_RWSCF, 'unpaired');
        title(['p = ', num2str(stats.p), '; effectSize = ', num2str(stats.effectSize), '; ' stats.normality]);
        hold off;
    end
   % ylim(ylimit);
