function getslopes_bymouse(mottype, allData, tracetype, BT_mice, RWS_mice, RWSCF_mice, RWSCF_PV_JAWS_mice, control_PV_JAWS_mice, RWSCF_PV_Gi_mice, control_PV_Gi_mice, RWSCF_SST_Gi_mice, control_SST_Gi_mice, RWS_SST_Gq_mice, control_SST_Gq_mice, norm)
...
% FIND SLOPES FROM TRIAL-AVERAGED AND CELL-AVERAGED (WITHIN MICE) TRACES
pre = calculateslopes(pre);
pre_wo = calculateslopes(pre_wo);

RWS_pre = calculateslopes(RWS_pre);
%RWS_pre_wo = calculateslopes(RWS_pre_wo);
RWS_post = calculateslopes(RWS_post);

RWSCF_pre = calculateslopes(RWSCF_pre);
%RWSCF_pre_wo = calculateslopes(RWSCF_pre_wo);
RWSCF_post = calculateslopes(RWSCF_post);

RWSCF_PV_JAWS_pre = calculateslopes(RWSCF_PV_JAWS_pre);
%RWSCF_PV_JAWS_pre_wo = calculateslopes(RWSCF_PV_JAWS_pre_wo);
RWSCF_PV_JAWS_post = calculateslopes(RWSCF_PV_JAWS_post);

control_PV_JAWS_pre = calculateslopes(control_PV_JAWS_pre);
%control_PV_JAWS_pre_wo = calculateslopes(control_PV_JAWS_pre_wo);
control_PV_JAWS_post = calculateslopes(control_PV_JAWS_post);

RWSCF_PV_Gi_pre = calculateslopes(RWSCF_PV_Gi_pre);
%RWSCF_PV_Gi_pre_wo = calculateslopes(RWSCF_PV_Gi_pre_wo);
RWSCF_PV_Gi_pre_dcz = calculateslopes(RWSCF_PV_Gi_pre_dcz);
RWSCF_PV_Gi_post = calculateslopes(RWSCF_PV_Gi_post);

control_PV_Gi_pre = calculateslopes(control_PV_Gi_pre);
%control_PV_Gi_pre_wo = calculateslopes(control_PV_Gi_pre_wo);
control_PV_Gi_pre_dcz = calculateslopes(control_PV_Gi_pre_dcz);
control_PV_Gi_post = calculateslopes(control_PV_Gi_post);

RWSCF_SST_Gi_pre = calculateslopes(RWSCF_SST_Gi_pre);
%RWSCF_SST_Gi_pre_wo = calculateslopes(RWSCF_SST_Gi_pre_wo);
RWSCF_SST_Gi_pre_dcz = calculateslopes(RWSCF_SST_Gi_pre_dcz);
RWSCF_SST_Gi_post = calculateslopes(RWSCF_SST_Gi_post);

control_SST_Gi_pre = calculateslopes(control_SST_Gi_pre);
%control_SST_Gi_pre_wo = calculateslopes(control_SST_Gi_pre_wo);
control_SST_Gi_pre_dcz = calculateslopes(control_SST_Gi_pre_dcz);
control_SST_Gi_post = calculateslopes(control_SST_Gi_post);

RWS_SST_Gq_pre = calculateslopes(RWS_SST_Gq_pre);
%RWS_SST_Gq_pre_wo = calculateslopes(RWS_SST_Gq_pre_wo);
RWS_SST_Gq_pre_dcz = calculateslopes(RWS_SST_Gq_pre_dcz);
RWS_SST_Gq_post = calculateslopes(RWS_SST_Gq_post);

control_SST_Gq_pre = calculateslopes(control_SST_Gq_pre);
%control_SST_Gq_pre_wo = calculateslopes(control_SST_Gq_pre_wo);
control_SST_Gq_pre_dcz = calculateslopes(control_SST_Gq_pre_dcz);
control_SST_Gq_post = calculateslopes(control_SST_Gq_post);
...
function var = calculateslopes(var)
    for i = 1:size(var, 2)
        temp = var{i};
        slope = nan(size(temp, 2), 1);
        for j = 1:size(temp, 2)
            trace = temp(310:402, j);
            if ~isnan(trace(1))
                [maximum, idx] = max(trace);
                val1 = trace(1);
                slope(j) = (maximum-val1)/(idx);
            end
        end
        var{i} = slope;
    end
