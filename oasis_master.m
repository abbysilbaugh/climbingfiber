...
% Run from Abby folder. Note: no other folders can exist inside analysis folder besides 'figs'

addpath('20230516_analysis');

allData = compileDataset; 

%% Manually identify artifact frames as needed

%removeartifact

%% Linearly interpolate artifact and generate trace

[allData] = interpartifact(allData);

%% Background subtract, smooth, and express as dF/F0
artifact_win = [310, 317]; % Not using as of 3/19/24
b_percent = 5; % percentile of trace for each ROI that will be used as F0 (Helmchen uses 1st)
smooth_win = 5; % window for smoothing with filter (Helmchen uses 500 msec/15 frames)

%[allData] = getGOLAY(allData, artifact_win, b_percent, smooth_win);
% golaysignal


[allData] = getGOLAY2(allData, b_percent, smooth_win);
%% Calculate baseline noise (σ) and SNR
% nMice x 1 cell arrays, each cell containing (nCells x 1) cell array
noise_prc = 5; % percentile of STD that will be used for baseline noise
sig_prc = 95; % percentile of trace over entire session to be used as numerator

[baselineNoise, SNR] = getbaselinenoise2(allData, noise_prc, sig_prc);
% baselineNoise, SNR

%% Plot distribution of SNRs and σ for each cell type
snrThreshold = 5; % arbitrary threshold to calculate number of neurons with high SNR

[SNRdist] = getSNRdist(allData, SNR, baselineNoise, 21, snrThreshold);

%% Run oasis (and plot all trials separately if desired...)
plot_all = false;
sn_thresh = 2; % sn_thresh*baseline noise is used in oasis function

[allData] = runoasis(allData, baselineNoise, plot_all, sn_thresh);
% golayshift, c_oasis, s_oasis, spiketimes, b_oasis, smin_oasis, sn_oasis

%% Find rest and running periods within trials
rest_period = 12; % length of period to be considered rest (Helmchen uses 400 msec, approx 12 frames)

allData = getrest(allData, rest_period);
% rest, run

%% Correlate each cell with motion (across all trials and separated by pre and post trials)
...
stim_onset = 310;
peaks_thr = 0.09; % min peak height % CHANGED FROM 0.2 02/11/24
%[allData] = getpeaks(allData, peaks_thr, stim_onset);

% Calculate peaks using baseline noise instead
peaks_thr = 3;
[allData] = getpeaks2(allData, peaks_thr, baselineNoise, stim_onset);
...
frame_win = [310, 341]; % 81 frames is ~ 2.614 s CHANGED FROM 391, 342 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! (372)
[allData] = getmean(allData, frame_win);
% adds mean_fluo_in_win and mean_num_mot to allData (nCells x nTrials)

frame_win_before = [300, 310];
[allData] = getmeanbefore(allData, frame_win_before);

[allData] = getmeandiff(allData, frame_win_before, frame_win);
...
plotalltraces_troubleshoot(allData, 1, 20) % select range of mice
%% Exclusion criteria
...
peak_win = {false, 10};
spike_win = {true, 3}; % CHANGED FROM 10 Nov 5, CHANGED FROM 4 2/11/24 (4)


[allData] = excludecellstraces(allData, SNR, SNRthresh, SNRmin, delta_b_oasis_thresh, trace_b_thresh, spike_after_peak, peak_win, spike_win);
...
for i = 1:length(plot_motType)
    ...
    prebar_multiplot(allData, plot_cellType{j}, 'all', plot_motType{i}, 'mean_fluo_diff', 'firstpeakamp', exclude, false, gettitle, savename{i,j}, ylimit)
    end
end
...
exclude = false;
includeopto = false;
norm = true; % note it doesn't make much sense to norm? or norm to w stim instead
...
% For Poster
plottime(allData, 'PN', 'all', 'all', 'b_oasis', exclude, 1, ylimit, xlimit, 'PN; NM Trials; All Expts', 'ABBY_PN_all_overtime_dreadd.jpg')
