...
% Run from Abby folder

%addpath('20230516_analysis');


% SPECIFY MOTION PARAMETERS
% If more than nMotion frames in Motion_win, trial is 'M' trial
% 1 frame   = 32.3 msec
% 3 frames  = 97 msec
% 6 frames  = 194 msec
% 8 frames  = 259 msec
...
% COMPILE DATASET
% Note: no other folders can exist inside analysis folder besides 'figs'
% allData.mouseIDs
% allData.data: 1 x nMice cell array. Each is a struct.

allData = compileDataset(nMotion, Motion_win);
% adds experimentType, cellType (nCells x 1); trialType, tetPeriod,
% reltime, motionCT (nTrials x 1); motionInfo, signal, raw_signal (nFrames
% x nCells x nTrials)

%% Update DREADD expt types
load("ISDREADD.mat");

allData = updateDREADD(allData, ISDREADD);

    

%% Find rest and running periods within trials
% Find periods of 400 ms of rest (no motion detected in this period)
% 400 msec is approximately 12 frames
% Contains 1 if frame is contained in period of rest
% Opposite of rest is running

allData = getrest(allData, 12); % 12 frames is approx 400 msec
% adds rest and running (nFrames x nTrials)

%% Background subtract, smooth, and express as dF/F0
% adds golaysignal to allData variable (smoothed and baseline corrected)
artifact_win = [310, 317];
b_percent = 5;
smooth_win = 5; % approx 500 msec smooth window %%%%%%%%% SMOOTH WIN 15 FOR ALL EXPERIMENTS PRIOR TO 2/11/24



[allData] = getGOLAY(allData, artifact_win(1):artifact_win(2), b_percent, smooth_win);
% adds golaysignal (nFrames x nCells x nTrials)


%% Calculate baseline noise (σ) and SNR
% nMice x 1 cell arrays, each cell containing (nCells x 1) cell array
noise_prc = 5;
sig_prc = 95;

[baselineNoise, SNR] = getbaselinenoise(allData, noise_prc, sig_prc);

%% Plot distribution of SNRs and σ for each cell type
snrThreshold = 5;

nBins = 21; 
[SNRdist] = getSNRdist(allData, SNR, baselineNoise, nBins, snrThreshold);

%% Run oasis (and plot all trials separately if desired...)

plot_all = false;
sn_thresh = 2;

[allData] = runoasis(allData, baselineNoise, plot_all, sn_thresh);
% adds golayshift, c_oasis, s_oasis, spiketimes (nFrames x nCells x nTrials);
% b_oasis, smin_oasis, sn_oasis (nCells x nTrials)

%% Correlate each cell with motion (all trials)
% Choose which trace to correlate with
motion_type = 'motion'; % or 'motion' or 'running'
signal_type = 'golayshift'; % 'c_oasis', 'spiketimes', 'golayshift', 'golaysignal', 'signal'

[allData] = getcellxmotion(allData, motion_type, signal_type);
% adds cellxmot (nCells x 1)

%% Correlate each cell with motion (separated by pre and post trials)
[allData] = cellxmotion_prepost(allData);
% adds cellxmot_pre and cellxmot_post

%% Calculate spike rate and mean dF/F0 for spontaneous rest/run conditions

spontFrames = true(620, 1);
spontFrames(310:372) = false;

[allData] = getspontinfo(allData, spontFrames);
% adds spont_rate, spont_rate_running, spont_rate_rest, mean_fluo_running,
% mean_fluo_rest (nCells x nTrials); modulationidx (nCells x 1)

% Notes: spont_rate_running, spont_rate_rest, mean_fluo_running,
% mean_fluo_rest for a given trial may contain NaNs if there were no rest
% or run periods, respectively

%% Plot distribution of correlations w/ motion for each cell type
nBins = 50;


corrthresh = .05                                                                                                                                                            ;

mottype = 'correlation'; % 'correlation' or 'modidx'

usetype = 'PRE'; % 'PRE', 'POST', or 'all' 
% using post doesn't make sense because they are pooled over both mouse types currently
% can't use this option if we want to use modidx, need to recalculate
[distcorr] = getcorrdist(allData, corrthresh, nBins, mottype, usetype);

[allData] = getmotcells(allData, corrthresh, mottype);
% adds cellmotType (nCells x 1)

%% Calculate mean dF/F0 and amount of motion for a specific window

frame_win = [310, 372]; % 81 frames is ~ 2.614 s CHANGED FROM 391, 342 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! (372)
[allData] = getmean(allData, frame_win);
% adds mean_fluo_in_win and mean_num_mot to allData (nCells x nTrials)

%% Find/Update M, NM, T, H trials

pre_win = [295 310]; % 15 frames is ~ 480 msec % START 280
post_win = [310 372]; % 62 frames is ~ 1.984 s

T_thresh = 1; % changed 5 from 5 Nov 5!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! (2)
M_thresh = 1; %
H_thresh = 3;

[allData] = gettransition(allData, pre_win, post_win, T_thresh, M_thresh, H_thresh);
% adds motdif to allData (nTrials x 1)

close all

%% Calculate tetanization spike rate and avg fluo during tet
% need to remove artifact in this before analysis

[allData] = gettetinfo(allData);
% adds tet_motion_rate (integer), tet_spike_rate, tet_mu_fluo,
% tet_stdev_fluo (nCells x 1)

%% Find peaks

stim_onset = 311;
peaks_thr = 0.09; % min peak prominence % CHANGED FROM 0.2 02/11/24
[allData] = getpeaks(allData, peaks_thr, stim_onset);
% amps, pkLocs, firstspike, firstpeaktime, firstpeakamp, firstpeakprom,
% firstpeakwidth (nCells x nTrials)

% Note: contains NaNs if data doesn't exist

%% GUI to visualize traces
oasisGUI(allData);
%% Plot all traces from single experiment

plotalltraces_troubleshoot(allData, 1, 19) % select range of mice
%% Exclusion criteria

SNRthresh = {false, 50};
delta_b_oasis_thresh = {false, 1};
trace_b_thresh = {false, 0.5};
spike_after_peak = {true}; % reject trace if peak occurs before spike % CHANGED FROM FALSE 2/11/24
peak_win = {false, 10};
spike_win = {true, 4}; % CHANGED FROM 10 Nov 5, CHANGED FROM 4 2/11/24 (4)


[allData] = excludecellstraces(allData, SNR, SNRthresh, delta_b_oasis_thresh, trace_b_thresh, spike_after_peak, peak_win, spike_win);
...
%% PLOT RHO SCATTER (1 variable by 1 variable) (4) !
...
for i = 1:length(plot_cellType)
    [pre_RWS, vartype] = grabdata(allData, plot_cellType{i}, 'all', 'all', 'all', 'all', 'cellxmot_pre', false);
    [cat_pre_RWS, ~] = catmice(allData, pre_RWS, vartype, 'whiskerTet');
    
    [pre_RWSCF, vartype] = grabdata(allData, plot_cellType{i}, 'all', 'all', 'all', 'all', 'cellxmot_pre', false);
    [cat_pre_RWSCF, ~] = catmice(allData, pre_RWSCF, vartype, 'whiskerOptoTet');
    
    [post_RWS, vartype] = grabdata(allData, plot_cellType{i}, 'all', 'all', 'all', 'all', 'cellxmot_post', false);
    [cat_post_RWS, ~] = catmice(allData, post_RWS, vartype, 'whiskerTet');
    
    [post_RWSCF, vartype] = grabdata(allData, plot_cellType{i}, 'all', 'all', 'all', 'all', 'cellxmot_post', false);
...
for i = 1:length(plot_cellType)
    [pre_RWS, vartype] = grabdata(allData, plot_cellType{i}, 'all', 'W', 'all', 'PRE', 'firstpeakamp', true);
    [pre_RWS, ~, vartype] = trialavg(pre_RWS, vartype);
    [cat_pre_RWS, ~] = catmice(allData, pre_RWS, vartype, 'whiskerTet');
    
    [pre_RWSCF, vartype] = grabdata(allData, plot_cellType{i}, 'all', 'W', 'all', 'PRE', 'firstpeakamp', true);
    [pre_RWSCF, ~, vartype] = trialavg(pre_RWSCF, vartype);
    [cat_pre_RWSCF, ~] = catmice(allData, pre_RWSCF, vartype, 'whiskerOptoTet');
    
    [post_RWS, vartype] = grabdata(allData, plot_cellType{i}, 'all', 'W', 'all', 'POST', 'firstpeakamp', true);
    [post_RWS, ~, vartype] = trialavg(post_RWS, vartype);
    [cat_post_RWS, ~] = catmice(allData, post_RWS, vartype, 'whiskerTet');
    
    [post_RWSCF, vartype] = grabdata(allData, plot_cellType{i}, 'all', 'W', 'all', 'POST', 'firstpeakamp', true);
...
%% PLOT PRE- BAR CHART MULTICOLOR (MULTIVARIABLE) (16) !
...
ylimit = [0 1.5]; % 1.5 is best to plot every motion type, 1.18 is best to plot 'all'

for i = 1:length(plot_motType)
    for j = 1:length(plot_cellType)
    prebar_multiplot(allData, plot_cellType{j}, 'all', plot_motType{i}, 'mean_fluo_in_win', 'firstpeakamp', true, false, gettitle, savename{i,j}, ylimit)
    end
end
...
for i = 1:length(plot_motType)
    for j = 1:length(plot_cellType)
    plotpretraces(allData, plot_cellType{j}, 'all', plot_motType{i}, 'golayshift', 'true', includeopto, ylimit, xlimit, norm, gettitle, savename{i, j})
    end
end
...
%% PLOT PRE-POST BAR CHART (MULTIVARIABLE)
...
% For Poster
plottime(allData, 'PN', 'all', 'all', 'firstpeakamp', false, 1, ylimit, xlimit, 'PN; NM Trials; All Expts', 'ABBY_PN_all_overtime_dreadd.jpg')

%% PLOT PRE-POST MOTION
plot_motType = {'all', 'NM', 'T', 'M'};
savename = {{'all_prepostmotion_RWS.tiff', 'all_prepostmotion_RWSCF.tiff'},{'NM_prepostmotion_RWS.tiff', 'NM_prepostmotion_RWSCF.tiff'}, {'T_prepostmotion_RWS.tiff', 'T_prepostmotion_RWSCF.tiff'}, {'M_prepostmotion_RWS.tiff', 'M_prepostmotion_RWSCF.tiff'}};
norm = false;
y_limit = [0 1];
...
plot_var = 'firstpeakprom';
exclude = true;

for i = 1:length(plot_motType)
    for j = 1:length(plot_cellType)

    [get_w, vartype] = grabdata(allData, plot_cellType{j}, plot_cellmotType, 'W', plot_motType{i}, 'PRE', plot_var, exclude);
    [get_w, ~, vartype] = trialavg(get_w, vartype);
    [cat_w, varcattype] = catmice(allData, get_w, vartype, 'all');
    
    [get_wo, vartype] = grabdata(allData, plot_cellType{j}, plot_cellmotType, 'WO', plot_motType{i}, 'PRE', plot_var, exclude);
    [get_wo, ~, vartype] = trialavg(get_wo, vartype);
    [cat_wo, varcattype] = catmice(allData, get_wo, vartype, 'all');
    
    
    getscatter(cat_w, cat_wo, 'Mean Fluo.', 'W', 'W-CF', plot_cellType{j}, savename{i, j})
    end
end
...
for i = 1:length(plot_cellType)
    % takes in getvarcat
    [getvar, vartype] = grabdata(allData, plot_cellType{i}, 'all', 'all', 'all', 'all', 'cellxmot_pre', false);
