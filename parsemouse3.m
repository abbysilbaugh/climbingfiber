function [pre_comb, post] = parsemouse3(motiontype, celltype, allData, mouse, choosesignal, baselineshift, baselineshiftperiod, baseline_reject_thresh, trialavg)
...
if strcmp(motiontype, 'all')
    pre_comb_trials = strcmp(tetPeriod, 'PRE') & strcmp(trialType, 'W');
    post_tet_trials = strcmp(tetPeriod, 'POST') & strcmp(trialType, 'W');
else
    pre_comb_trials = strcmp(tetPeriod, 'PRE') & strcmp(trialType, 'W') & strcmp(motionType, motiontype);
...
if length(size(signal)) == 3
    ...
    pre_comb = signal(:, cells, pre_comb_trials);
