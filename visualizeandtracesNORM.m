...
function [pre_norm, post_norm, pre2_norm, post2_norm] = visualizeandtracesNORM(neuron_colors, xlimits, ylimits, stimframe,...
    pre_norm, pre_dcz, post_norm, ...
    pre2_norm, pre_dcz2, post2_norm, showtraces)
...
pre_selected = pre_norm{i};
pre_dcz_selected = pre_dcz{i};
post_selected = post_norm{i};
pre2_selected = pre2_norm{i};
pre_dcz2_selected = pre_dcz2{i};
post2_selected = post2_norm{i};
...
% Compute mean and standard deviation for each cell (column-wise)
mu_pre = mean(pre_selected, 1);  % Mean for each cell
sigma_pre = std(pre_selected, 0, 1);  % Standard deviation for each cell

mu_pre2 = mean(pre2_selected, 1);  % Mean for pre2_selected
sigma_pre2 = std(pre2_selected, 0, 1);  % Standard deviation for pre2_selected

% Normalize using Z-score: (X - mean) / std
pre_dcz_selected = (pre_dcz_selected - mu_pre) ./ sigma_pre;
post_selected = (post_selected - mu_pre) ./ sigma_pre;

pre_dcz2_selected = (pre_dcz2_selected - mu_pre2) ./ sigma_pre2;
post2_selected = (post2_selected - mu_pre2) ./ sigma_pre2;


pre_norm{i} = pre_dcz_selected;
post_norm{i} = post_selected;
pre2_norm{i} = pre_dcz2_selected;
post2_norm{i} = post2_selected;
