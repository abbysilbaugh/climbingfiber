function DREADDtraces(allData, cellType, cellmotType, trialmotType, variable, exclude, savename, gettitle, norm, ylimit)
...
[pre_cat, varcattype] = catmice(allData, pre, pre_vartype, 'whiskerTet_DREADD');
[pre_cat2, varcattype2] = catmice(allData, pre, pre_vartype, 'whiskerOptoTet_DREADD');

[DCZ, DCZ_vartype] = grabdata(allData, cellType, cellmotType, 'W', trialmotType, 'PRE',  'DCZ', variable, exclude);
[DCZ, DCZ_vartype] = trialpool(DCZ, DCZ_vartype);
[DCZ_cat, varcattype] = catmice(allData, DCZ, DCZ_vartype, 'whiskerTet_DREADD');
[DCZ_cat2, varcattype2] = catmice(allData, DCZ, DCZ_vartype, 'whiskerOptoTet_DREADD');

[post, post_vartype] = grabdata(allData, cellType, cellmotType, 'W', trialmotType, 'POST',  'DCZ', variable, exclude);
[post, post_vartype] = trialpool(post, post_vartype);
[post_cat, varcattype] = catmice(allData, post, post_vartype, 'whiskerTet_DREADD');
[post_cat2, varcattype2] = catmice(allData, post, post_vartype, 'whiskerOptoTet_DREADD');
...
DCZ_mean = mean(DCZ_cat, 2, 'omitnan'); DCZ_sem = std(DCZ_cat, 0, 2, 'omitnan') / sqrt(size(DCZ_cat, 2));
DCZ_mean2 = mean(DCZ_cat2, 2, 'omitnan'); DCZ_sem2 = std(DCZ_cat2, 0, 2, 'omitnan') / sqrt(size(DCZ_cat2, 2));
...
if norm
    ...
    shift_DCZ_mean = mean(DCZ_mean(300:304)); %pre_mean = pre_mean - shift_pre_mean;
    shift_DCZ_mean2 = mean(DCZ_mean2(300:304)); %pre_mean2 = pre_mean2 - shift_pre_mean2;
    ...
    if shift_DCZ_mean < 0
        DCZ_mean = DCZ_mean + shift_DCZ_mean;
    elseif shift_DCZ_mean > 0
        DCZ_mean = DCZ_mean - shift_DCZ_mean;
    end

    if shift_DCZ_mean2 < 0
        DCZ_mean2 = DCZ_mean2 + shift_DCZ_mean2;
    elseif shift_pre_mean > 0
        DCZ_mean2 = DCZ_mean2 - shift_DCZ_mean2;
    end
...
% RWS EXPERIMENT
    % Plot W PRE (no DREADD) (whiskerTet)
    ...
    % Plot W PRE (DREADD) (whiskerTet)
    upperBound = DCZ_mean + DCZ_sem;
    lowerBound = DCZ_mean - DCZ_sem;
    x = 1:length(DCZ_mean);
    fill([x, fliplr(x)], [upperBound', fliplr(lowerBound')], 'k', ...
        'linestyle', 'none', 'FaceAlpha', semAlpha);
    hold on;
    PRE_D = plot(x, DCZ_mean, 'Color', 'k', 'LineWidth', 3, 'LineStyle', '-.');
    ...
    lgd = legend([PRE, PRE_D, POST], {'PRE', 'PRE_D', 'POST'});
...
% RWSCF EXPERIMENT
    ...
    PRE = plot(x, pre_mean2, 'Color', 'k', 'LineWidth', 2, 'LineStyle', '-');
     ...
     % Plot W PRE (DREADD) (whiskerTet)
    upperBound = DCZ_mean2 + DCZ_sem2;
    lowerBound = DCZ_mean2 - DCZ_sem2;
    x = 1:length(DCZ_mean2);
    fill([x, fliplr(x)], [upperBound', fliplr(lowerBound')], 'k', ...
        'linestyle', 'none', 'FaceAlpha', semAlpha);
    hold on;
    PRE_D = plot(x, DCZ_mean2, 'Color', 'k', 'LineWidth', 3, 'LineStyle', '-.');
