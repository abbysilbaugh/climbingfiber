function crosscorr_grant(allData, mouse, active_post_RWS, window)

    getcells = active_post_RWS(:, 2) == mouse;
    active_post_RWS = active_post_RWS(:, 1); active_post_RWS = active_post_RWS(getcells);
    ...
    w_pre = signal(window(1):window(2), active_post_RWS, w_pre);
    [~, nCells, nTrials_pre] = size(w_pre);
    w_post = signal(window(1):window(2), active_post_RWS, w_post);
