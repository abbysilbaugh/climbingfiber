function plotmax_bymouse(m1_cond1, m1_cond2, m2_cond1, m2_cond2, neuron_colors, plotlines, evoked_win, group_names, condition_names, ylimits)
...
ylabel('Amplitude (ΔF/F0)'); % Adjust based on what your data represents
ylim(ylimits)
    ...
    for i = 1:nMice
        for j = 1:ncellType
            ...
            if ~isempty(signal)
                ...
                Amplitude = NaN(nCells, nTrials);
                for k = 1:nCells
                    for t = 1:nTrials
                        temp = signal(:, k, t);
                            %Amplitude(k, t) = abs(trapz(temp));
                            Amplitude(k, t) = max(temp);
                    end
                end
                var{i, j} = Amplitude;
            end
