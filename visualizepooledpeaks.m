function visualizepooledpeaks(allData, celltype, responsivewindow, motiontrial)
...
for i = 1:nMice
    ...
    pkLocs = allData.data{i}.pkLocs(:, get_trials);
    ...
    if strcmp(celltype, 'all')
        ...
        pkLocs = pkLocs(getcells, :);
    end
        
        numcells = numcells + size(pkLocs, 1);
        for c = 1:size(pkLocs, 1)
            temp = pkLocs(c, :); temp = temp(:); temp = cell2mat(temp);
        ...
        pkvec = pkLocs(:); pkvec = cell2mat(pkvec); 
...
for i = mice
    ...
    if strcmp(celltype, 'all')
        ...
        numcells_W = numcells_W + size(pkLocs, 1);
        for c = 1:size(pkLocs, 1)
            temp = pkLocs(c, :); temp = temp(:); temp = cell2mat(temp);
            temp = find(temp > responsivewindow(1) & temp < responsivewindow(2));
            if length(temp) >= 1
...
for i = mice
    ...
    if strcmp(celltype, 'all')
        ...
        numcells_WO = numcells_WO + size(pkLocs, 1);
        for c = 1:size(pkLocs, 1)
            temp = pkLocs(c, :); temp = temp(:); temp = cell2mat(temp);
            temp = find(temp > responsivewindow(1) & temp < responsivewindow(2));
            if length(temp) >= 1
...
for i = mice
    ...
    if strcmp(celltype, 'all')
        ...
        numcells_preRWS = numcells_preRWS + size(pkLocs, 1);
        for c = 1:size(pkLocs, 1)
            temp = pkLocs(c, :); temp = temp(:); temp = cell2mat(temp);
            temp = find(temp > responsivewindow(1) & temp < responsivewindow(2));
            if length(temp) >= 1
...
for i = mice
    ...
    if strcmp(celltype, 'all')
        ...
        numcells_postRWS = numcells_postRWS + size(pkLocs, 1);
        for c = 1:size(pkLocs, 1)
            temp = pkLocs(c, :); temp = temp(:); temp = cell2mat(temp);
            temp = find(temp > responsivewindow(1) & temp < responsivewindow(2));
            if length(temp) >= 1
...
for i = mice
    ...
    if strcmp(celltype, 'all')
        ...
        numcells_preRWSCF = numcells_preRWSCF + size(pkLocs, 1);
        for c = 1:size(pkLocs, 1)
            temp = pkLocs(c, :); temp = temp(:); temp = cell2mat(temp);
            temp = find(temp > responsivewindow(1) & temp < responsivewindow(2));
            if length(temp) >= 1
...
for i = mice
    ...
    if strcmp(celltype, 'all')
        ...
        numcells_postRWSCF = numcells_postRWSCF + size(pkLocs, 1);
        for c = 1:size(pkLocs, 1)
            temp = pkLocs(c, :); temp = temp(:); temp = cell2mat(temp);
            temp = find(temp > responsivewindow(1) & temp < responsivewindow(2));
            if length(temp) >= 1
